...
const MenuHumanChain = () => {
  ...
  let initUSB = async () => {
    ...
    sensorSet((state)=> {return {...state,port:port}})
  };
  ...
  async function readUntilClosed(port) {
    ...
    while (port.readable && keepReading) {
      ...
      if (!reader) {
        ...
        sensorSet((state)=> {return {...state,reader:reader}})
      }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          style={{ width: "50vw", marginBlock: "10px" }}
          onClick={() => sensorSet((state)=>{return {...state,Sensorname:'pedro martinez'}})}
        >
         Cambiar nombre
        </button>
        ...
        <button disabled={!sensorData?.port}
          onClick={() => openComPort(sensorData.port)}
        >
         Open Port 
        </button>

        <button disabled={!sensorData?.port?.readable}
        ...
        <div>
          <p>Puerto: {JSON.stringify(sensorData?.port?.getInfo()) || 'Ninguno'} </p>
